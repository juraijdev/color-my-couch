...
const userPrompt = "Analyze this furniture image and identify ALL distinct parts of THE FURNITURE ONLY — exclude any background (walls, floors, scenery). Cover the FULL furniture from LEFT-MOST edge to RIGHT-MOST edge and from top to bottom — do NOT cut, crop, or skip any portion, regardless of whether the buffet table is small, medium, large, wide, long, or landscape-oriented. The furniture's exact shape, proportions, dimensions, and construction must be preserved EXACTLY as shown in the design — do NOT redraw, re-imagine, or alter any silhouette, edge, or proportion. Bounding boxes must trace the furniture as actually drawn. For LARGE / LANDSCAPE / WIDE buffet tables, the Top Surface, Front Panel, Shelf Wood, Frame/Legs, and Stainless Steel Trim & Edges bounding boxes MUST span the FULL horizontal extent of those parts (often 80-100% of the image width) — never return half-width boxes. Apply these GROUPING RULES: (1) Combine ALL top surface modules into ONE 'Top Surface' part across the full width, but EXCLUDE any metal side panels, side-return panels, vertical top fascia, perimeter lips, side lips, front lips, edge caps, or divider strips from the top surface. If the top is stone, marble, granite, quartz, terrazzo, solid-surface, or similar slab material, it is still the same single 'Top Surface' part and must cover the full visible countertop without cropping. (2) For buffet tables and serving stations, group ALL top divider strips plus the matching front/side top metal edge lips, small vertical top side panels, side-return panels, outer end caps, and perimeter top trim into ONE 'Stainless Steel Trim & Edges' part if they share the same finish — even on long landscape buffet tables with many modules. Do NOT split identical dividers into multiple parts. (3) The thin top front/side metal lip and the small vertical top side panel / side-return panel must always be grouped with the divider strips when they visually belong to the same trim system. (4) The top side edge metal and the divider metal must stay in the same grouped part so the user can recolor them together. (5) Combine ALL wooden shelves FULLY (all faces — top, front edge, bottom, sides) into ONE 'Shelf Wood' part across the full width. (6) CRITICAL — FRONT DECORATIVE PANEL DETECTION (applies to ALL buffet tables — small, large, wide, landscape): Carefully inspect the entire front face of the buffet table. The plain/uniform wood/stone front fascia is 'Front Panel'. BUT if the front contains ANY visually distinct sub-element — different pattern, different wood grain direction, different color, different finish, different material, contrasting tone, decorative cutout, wavy/curved cut-out exposing a darker or lighter layer beneath, slatted screen, lattice, perforated panel, rattan/cane insert, contrasting wood inlay, marquetry, fluted/reeded section, painted accent panel, mirror panel, fabric/leather panel, distinct door/drawer faces, carved panel, geometric pattern panel, two-tone front, upper/lower split with different finishes — each visually distinct sub-element MUST be returned as its OWN separate part with a DESCRIPTIVE name like 'Front Decorative Screen', 'Front Inlay', 'Front Fluted Panel', 'Front Accent Panel', 'Front Carved Panel', 'Front Pattern Panel', 'Front Two-Tone Insert', 'Front Lower Accent', 'Front Upper Decorative'. Do NOT name them generically as 'Front Panel Upper' / 'Front Panel Lower' — use a descriptive decorative name so they are recognized as decorative sub-parts. If a long buffet table has the same decorative pattern repeated across multiple modules, group them as ONE part covering the full horizontal span. If there are TWO or more visibly different decorative finishes side-by-side or stacked, each is its own part. NEVER merge visually different front sub-elements into a single 'Front Panel'. (7) Identify ALL other parts: frame/legs (full width), side panels, back panel, hardware, wheels, doors, drawers, decorative elements. Thin metal trims and the small top side panel are critical parts and must not be skipped. (8) If the broad top wood/stone/marble/granite/quartz and the surrounding side/divider trim are visibly different materials, they MUST be returned as different parts every time. Do NOT skip any visible furniture part or any decorative sub-part on the front, regardless of furniture size. Preserve the exact furniture shape, dimensions, and construction logic. Aim for 5-12 well-grouped parts. Return the JSON structure.";
